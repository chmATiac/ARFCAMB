    ...
    real(dl) counts_vlos_source ! [CHM] vlos as additional observable
    ...
    !CHM ----
    external lum_z
    real(dl) c_light, lum_z, term1, term2, term3, term4
    ...
    ![CHM, old-2]            sources(3+w_ix) =-2*phi*W%win_lens(j) + CP%AIA*0.0134*(CP%omch2+CP%ombh2)/(CP%H0/100)/(CP%H0/100) * \
    !            (1/a)**(CP%epsIA)*lum_z(1/a-1.)**(CP%betaIA) * W%wing(j)*clxc / W%GFmatter2(j) / (k*chi)**2! July 5th 2024
    ![CHM, old-1]            sources(3+w_ix) =-2*phi*W%win_lens(j) + CP%AIA*0.0134*(CP%omch2+CP%ombh2)/(CP%H0/100)/(CP%H0/100) * \
    ...
    !                sources(3+W%mag_index+State%num_redshiftwindows) = phi*W%win_lensZ(j)*(2-5*W%Window%dlog10Ndm)
            ...
            elseif ((W%kind == window_counts).or.(W%kind == window_arf)) then ![CHM] original ADF part, that now should also work for ARF
                ...
                if ((CP%SourceTerms%counts_lensing).and.(W%kind== window_arf)) &
                  &   sources(3+W%mag_index+State%num_redshiftwindows) = phi*W%win_lensZ(j)*(2-5*W%Window%dlog10Ndm)
            elseif (W%kind == window_vlos) then  ! [CHM]
                ![CHM] Here we include the vlos term simply weighted by the density window, it is similar to the velocity term
                ! [Jan 2025] - we have just noticed there's a minus sign that must be included: the velocity term is ~ W_\delta \eta (-\vv\cdot \hat{n})/c.
                !   This can be easily seen by comparing Eqs. 37 and 43 in Challinor & Lewis 2011 on CAMB sources
                counts_vlos_source= (-1) * ( (-2.D0*W%wing(j)*adotoa+W%dwing(j))/k*sigma &
                        +W%wing(j)*etak/k/EV%Kf(1) )
                sources(3+w_ix)= counts_vlos_source /W%Fq
                ! ---  [CHM] vlos as additional observable
