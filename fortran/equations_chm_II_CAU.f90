    ...
    !
    ![CHM] Here we introduce the function <L>(z)/L_{\rm star}(z) for Euclid galaxies provided by Stephane Ilic (June 2023)
    function lum_z(z)
    !
    use results
    use DarkEnergyInterface
    ...
    real(dl), intent(in) :: z
    real(dl) lum_z
    real(dl), dimension(220) :: zar, lar
    integer i_mem, iline, izeff(1)
    data i_mem/0/
    save i_mem,zar,lar
    !
    if (i_mem.eq.0) then 
        open(30,file='scaledmeanlum-E2Sa.dat',status='unknown')
        do iline=1,220
            read(30,*) zar(iline), lar(iline)
        enddo
        close(30)
        i_mem=1
    endif
    !
    izeff=MINLOC(abs(z-zar))
    lum_z=lar( izeff(1) )
    !
    end function lum_z
    ![CHM] _________________
    ![CHM] We next introduce some geometric function related to the spherical top hat window function in Fourier space:
    !  W_k^{TH,k\eta} := W_k^{TH,chi} = 4\pi k^-3 \int_0^chi dy y^2 j_0(y)
    ! We next compute Z(chi) = \int_0^chi dy y^2 j_0(y)
    function Zint(nchi,chi)
    !
    ! chi must be a real array of length nchi !!
    !
    implicit none
    !
    integer, parameter :: npoints=1000
    integer :: iz,jchi(1),i_mem
    !
    integer, intent(in) :: nchi
    real(dl), intent(in) :: chi(nchi)
    real(dl), intent(out) :: Zint(nchi)
    real(dl) zmin,zmax,zchi(nchi)
    real(dl), dimension(npoints) :: zarr, dzarr, ztab
    !
    data i_mem/0/
    save i_mem,ztab,zarr,pi_const
    !
    !
    if (i_mem.eq.0) then
        zmin=-4._dl
        zmax=5._dl
        do iz=1,npoints
            zarr(iz)=zmin  + (zmax-zmin)/(npoints-1)*(iz-1)
        enddo
        do iz=2,npoints-1
            dzarr(iz)=0.5*(zarr(iz+1)-zarr(iz))
        enddo
        dzarr(1)=0.5*(zarr(2)-zarr(1))
        dzarr(npoints)=0.5*(zarr(npoints)-zarr(npoints-1))
        !
        do iz=1,npoints
            ztab(iz)=sum(dzarr(1:iz)*exp(3*zarr(1:iz))*sin( exp(zarr(1:iz))) / exp(zarr(1:iz)) )
        enddo
        i_mem=1
    endif
    !
    zchi=log(abs(chi))
    do iz=1,nchi
        jchi=MINLOC(abs(zchi(iz)-zarr))
        Zint(iz)=ztar(jchi(1))
    enddo
    !
    end function Zint
    ![CHM] ___________________________________
    !
    ![CHM] We next compute Gconv(k,xi,chi) : = \int_-1^1 dmu (4\pi) |\vk-\vq|^-3 Zint(|\vk-\vq| x chi)
    !   xi := q/k
    function Gconv(k,xi,chi)
    !
    !k,xi, and chi should be real scalars
    !
    implicit none
    !
    integer, parameter :: nmuar=100
    integer :: imem,imu
    real(dl) :: dmuar,pi_const
    real(dl), dimension(nmuar) :: muar
    real(dl), intent(in) :: k,xi,chi
    real(dl) :: Gconv
    !
    data i_mem/0/
    save i_mem,muar,dmuar,pi_const
    !
    if (i_mem.eq.0):
        do imu=1,nmuar 
            muar(imu)=-1 + 2./(nmuar-1)*(imu-1)
        enddo
        dmuar=2./(nmuar-1)
        pi_const= 4_dl* datan(1_dl)
        i_mem=1
    endif
    !
    Gconv=4*pi_const/k/k/k* sum(dmuar*(1+xi*xi-2*xi*muar)**(-1.5_dl)*Zint(k*sqrt(1._dl+xi*xi-2*xi*muar)*chi) )
    !
    end function Gconv
    ...
    do w_ix = 1, State%num_redshiftwindows 
        associate (W => State%Redshift_W(w_ix))
            ...
            if (W%kind == window_lensing) then
                ...
                !!print*,'redshift, ks, growth factor: ',1/a-1, k, k_i(jink),abs(a/a_i(jink)*phi/phi_i(jink)*1./501.)
                sources(3+w_ix) =-2*phi*W%win_lens(j) + CP%AIA*0.0134*(CP%omch2+CP%ombh2)/(CP%H0/100)/(CP%H0/100) * \
                (1/a)**(CP%epsIA)*lum_z(1/a-1.)**(CP%betaIA) * W%wing(j)*clxc / W%GFmatter2(j) / (k*chi)**2! July 5th 2024
    ![CHM, old]      sources(3+w_ix) =-2*phi*W%win_lens(j) + CP%AIA*0.0134*(CP%omch2+CP%ombh2)/(CP%H0/100)/(CP%H0/100) * \
    !                (1/a)**(CP%epsIA)*lum_z(1/a-1.)**(CP%betaIA) * W%wing(j)*clxc / abs(phi/0.6*a) / (k*chi)**2!* adotoa/a
                    !print*,'TSTs = ',1/a-1,(phi/0.6*a),lum_z(1/a-1.)**(CP%betaIA)!adotoa*a,CP%H0,adotoa/a*c_light/CP%H0
    ...
    !            if (CP%SourceTerms%counts_density) then
                    !chm
                    !print*,'TST EV',1/a-1, k, phi/clxc*a*k*k,-1.5*(CP%omch2+CP%ombh2)*100**2/c_light/c_light!1/a-1.,k,phi !associated(EV%OutputTransfer)!EV%OutputTransfer(Transfer_tot)
                    !if (w_ix.eq.1) print*,'TST EV:', 1/a-1, k, phi/clxc*a*k*k,-1.5*(CP%omch2+CP%ombh2)*100**2
                    ...
                    ![CHM] Now we introduce f_NL in bias defintion:
    !                counts_density_source= (1/a -1 - W%z_prom) *  W%wing(j)*(clxc*W%Window%GetBias(k,a) * &
    !                 (1+(W%Window%GetBias(k,a)-1.)/W%Window%GetBias(k,a)*3*1.686*CP%f_NL*(-3./5.)/(phi*k*k*a) &
    ...
    !                sources(3+W%mag_index+State%num_redshiftwindows) = phi*W%win_lensZ(j)*(2-5*W%Window%dlog10Ndm)
            ...
            elseif ((W%kind == window_counts).or.(W%kind == window_arf)) then ![CHM] original ADF part, that now should also work for ARF
                !assume zero velocity bias and relevant tracer is CDM perturbation
                !neglect anisotropic stress in some places

                !Main density source
                if (CP%SourceTerms%counts_density) then
                    !chm ___ Modifying expression to include f_NL ___
                    !counts_density_source= W%wing(j)*(clxc*W%Window%GetBias(k,a) + (W%comoving_density_ev(j) - 3*adotoa)*sigma/k)
                    !Newtonian gauge count density; bias assumed to be on synchronous gauge CDM density
                    ![CHM] Now we introduce f_NL in bias definition:
                    biasEff=W%Window%GetBias(k,a)
                ...
                sources(3+w_ix)=sources(3+w_ix)/W%Fq ! [CHM] - For ARF W%Fq was close to zero! (before redefining W%Fq)
                if ((CP%SourceTerms%counts_lensing).and.(W%kind== window_counts)) &
                  &  sources(3+W%mag_index+State%num_redshiftwindows) = phi*W%win_lens(j)*(2-5*W%Window%dlog10Ndm)
                if ((CP%SourceTerms%counts_lensing).and.(W%kind== window_arf)) &
                  &   sources(3+W%mag_index+State%num_redshiftwindows) = phi*W%win_lensZ(j)*(2-5*W%Window%dlog10Ndm)
