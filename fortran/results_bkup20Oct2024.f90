    ...
    do i = 1, State%num_redshiftwindows
	    ...
	    !
	        if (RedWin%kind == window_arf) then 
            	print*,'V4 ARF, window_i  = ',i!,RedWin%z_prom 
            else
            	print*,'V4 ADF or lensing, window_i = ',i
            endif
            !ARF------------------------------------------------------
            ...
            elseif ( (RedWin%kind == window_counts) .or. (RedWin%kind == window_arf) ) then
                if (State%CP%SourceTerms%counts_evolve) then ! [CHM] For ARF we assume there's no source evolution since we generally "select" a subsample of dNdz
