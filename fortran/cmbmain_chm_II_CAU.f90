    ...
    if (CP%WantCls) then
        ...
        !$OMP END PARALLEL DO
        !![CHM] -- I THINK CAUSALITY ON SOURCES SHOULD BE INTRODUCED HERE!
        !! We need a loop in source type and time, and for every time, use "tau" == conformal time and conduct convolution in k (i_qx)
        allocate(auxSources(ThisSources%SourceNum,ThisSources%Evolve_q%npoints)
        allocate(qarr(ThisSources%Evolve_q%npoints),dqarr(ThisSources%Evolve_q%npoints))
        allocate(auxSources2(ThisSources%SourceNum,ThisSources%Evolve_q%npoints))
        do i2=1,State%TimeSteps%npoints 
                tau=TimeSteps%points(i2)
                auxSources(:,:)=ThisSources%LinearSrc(:,:,i2)
                qarr=ThisSources%Evolve_q%points ; nq=ThisSources%Evolve_q%npoints
                do i3=2,nq-1
                    dqarr(i3)=0.5*(qarr(i3+1)-qarr(i3-1))
                enddo
                dqarr(1)=0.5*(qarr(2)-qarr(1))  ; dqarr(nq)=0.5*(qarr(nq)-qarr(nq-1))
                do i3=1,nq
                    auxSources2(:,i3)=0
                    do i1=1,nq
                        auxSources2(:,i3)=auxSources2(:,i3)+ &
                            auxSources(:,i1)*dqarr(i1)*qarr(i1)*qarr(i1)*Gconv(qarr(i1),qarr(i3)/qarr(i1),tau)
                    enddo
                enddo
                ThisSources%LinearSrc(:,:,i2)=auxSources2(:,:)
        enddo
        !![CHM] _________________________
...
101         if (CP%WantTransfer.and.itf <= State%num_transfer_redshifts) then
    ...
    !![CHM] - here we introduce the LOS smearing due to photo-z errors ---
    !if (CP%photo_z.eq..true.) then
    !    allocate(LinearSrcAux(ThisSources%SourceNum,State%TimeSteps%npoints))
    !    do j=2,State%TimeSteps%npoints
    !         tauend=State%TimeSteps%points(j)
    ...
    !            LinearSrcAux(:,j)=0
    !        else
    !            LinearSrcAux(:,j)=ThisSources%LinearSrc(EV%q_ix,:,j)
    !endif 
    !![CHM] ____
