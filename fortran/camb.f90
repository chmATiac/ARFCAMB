    ...
    do i=1, num_redshiftwindows
        ...
        class is (TGaussianSourceWindow)
            ...
            elseif (S == 'arf') then ! ARF
                RedWin%source_type = window_arf ! ARF
            elseif (S == 'vlos') then ! [CHM] - vlos
                RedWin%source_type = window_vlos ! [CHM] - vlos
            ...
            if ( (RedWin%source_type == window_counts).or.(RedWin%source_type == window_arf) ) then
                DoCounts = .true.
                RedWin%bias = Ini%Read_Double_Array('redshift_bias', i)
                RedWin%dlog10Ndm = Ini%Read_Double_Array('redshift_dlog10Ndm', i ,0.d0)
            end if
            if (RedWin%source_type == window_lensing) then !! Introducing IA 
                call Ini%Read('lensing IA',P%SourceTerms%lensing_IA)
            endif
        class default
