    ...
    Type CalWins
        ...
        real(dl), allocatable :: GFmatter(:), dGFmatter(:) ! Matter density approx. growth factor (for IA computation in LCDM [only])
        real(dl) :: z_prom ! ARF
    end Type CalWins
    ...
    if (this%CP%WantScalars .and. this%CP%WantCls .and. this%num_redshiftwindows>0) then
        ...
        do nu_i = 1, this%num_redshiftwindows
            ...
            if ((Win%Window%source_type==window_lensing).and.(P%SourceTerms%lensing_IA) ) then !! [CHM] This adds the extra window for IA
                this%num_extra_redshiftwindows = this%num_extra_redshiftwindows + 1
                Win%mag_index = this%num_extra_redshiftwindows                
            endif !! [CHM]
            !$OMP END CRITICAL
    ...
    !CHM ----
    function CAMBdata_DmatterApproxofz(this,z)
    !Approximate linear density growth factor for LCDM with no coupling with baryons and no impact of radiation 
    !Not to be used at z>10! (expect few percent errors at best)
    !
    class(CAMBdata) :: this
    real(dl), intent(in) :: z
    integer, parameter :: nint=1000
    real(dl) :: zzar(nint), aar(nint), Hzarr(nint), gfar(nint), xint(nint),dGF
    real(dl) :: CAMBdata_DmatterApproxofz
    integer i_mem, ij, iz
    save i_mem, zzar, gfar
    data i_mem/0/
    !!
    if (i_mem.eq.0) then
        do ij = 1,nint 
            aar(ij)=1/(1.+20.*(nint-ij)/nint)
        enddo 
        zzar=1/aar-1 !!; dz=20/nint
        call CAMBdata_HofzArr(this, Hzarr, zzar, nint)
        xint=1/aar/Hzarr/Hzarr/Hzarr
        gfar=0
        do ij = 1,nint
            gfar(ij)=sum(xint(1:ij))/sum(xint) * Hzarr(ij)
        enddo 
        gfar=gfar / gfar(nint)
        i_mem=1
    endif
    !!
    iz=minloc(abs(z-zzar),dim=1)
    dGF= (gfar(iz)-gfar(iz-1) ) / (zzar(iz)-zzar(iz-1)) !! we assume iz!=1, z<20
    CAMBdata_DmatterApproxofz=gfar(iz)+dGF*(z-zzar(iz))
    !!
    end function  CAMBdata_DmatterApproxofz
    ...
    real(dl) awin_lens1Z(State%num_redshiftwindows),awin_lens2Z(State%num_redshiftwindows), zwindow ! ARF
    ...
    do RW_i = 1, State%num_redshiftwindows
        associate (RedWin => State%Redshift_w(RW_i))
            ...
            if (RedWin%kind == window_lensing .or.  (RedWin%kind == window_counts .or. RedWin%kind == window_arf) & ! [CHM] include arf
                ...
                allocate(RW(RW_i)%GFmatter(nthermo)) ! Growth factor for matter density (approx.) for IA estimation (correct for LCDM only)
                allocate(RW(RW_i)%dGFmatter(nthermo)) ! Growth factor for matter density (approx.) for IA estimation (correct for LCDM only)
            end if
        end associate
    end do
                    ...
                    if (a > 1d-4) then
                        ...
                        if  (RedWin%kind == window_lensing .or.  (RedWin%kind == window_counts .or. RedWin%kind == window_arf )  & ! [CHM] include arf
                            ...
                            if (State%tau0 - tau > 2) then
                                ...
                                ! ARF: 
                                awin_lens1Z(RW_i) = awin_lens1Z(RW_i) + adot * zwindow * dtau
                                awin_lens2Z(RW_i) = awin_lens2Z(RW_i) + adot * zwindow * dtau/(State%tau0-tau)
                                Win%awin_lensZ(i) = awin_lens1Z(RW_i)/(State%tau0-tau) - awin_lens2Z(RW_i)
                                ! Matter density growth factor (approx) for IA estimation in LCDM
                                Win%GFmatter(i) = CAMBdata_DmatterApproxofz(State,1/a-1)
                            else
                                Win%awin_lens(i) = 0
                        ...
                        if (RedWin%kind == window_lensing .or.  (RedWin%kind == window_counts .or. RedWin%kind == window_arf)&
                            ...
                            Win%awin_lens(i)=0
                            Win%awin_lensZ(i) = 0 ! ARF
                            Win%GFmatter(i) = 0 ! Matter density growth factor (approx) for IA estimation (LCDM only)
                        end if
                    end if
                end associate
            end do
    ...
    do RW_i = 1, State%num_redshiftwindows
        associate(Win => RW(RW_i))
            if (State%Redshift_w(RW_i)%kind == window_lensing .or. &
                ...
                !! Win%awin_lensZ=Win%awin_lensZ-Win%awin_lens*Win%z_prom! ARF (z-z_prom) is included in zwindow in this version
    ...
    if (State%num_redshiftwindows >0) then
        ...
        do j2 = 1, State%num_redshiftwindows
            if (State%Redshift_w(j2)%has_lensing_window) then
                ...
                call splder(RW(j2)%awin_lensZ,RW(j2)%dawin_lensZ,nthermo,spline_data) ! ARF
                call splder(RW(j2)%GFmatter,RW(j2)%dGFmatter,nthermo,spline_data) ! ARF
            end if
        end do
    end if
    ...
    if (State%num_redshiftwindows > 0) then
        ...
        do i=1,State%num_redshiftwindows
            ...
            !ARF
                ...
                allocate(Win%dwingZ(nstep))
                allocate(Win%GFmatter2(nstep)) !! approx. matter density linear growth factor for IA estimation (for LCDM only)
            !ARF ___
                ...
                !if (Win%kind == window_counts .or. Win%kind == window_arf) then ![CHM]
                if (Win%kind == window_counts .or. Win%kind == window_arf.or. Win%kind == window_lensing) then ![CHM] we need this for IA
    ...
    real(dl) window, winamp, zwindow, window_tmp
    ...
    do i = 1, State%num_redshiftwindows
        associate (RedWin => State%Redshift_W(i))
            ...
            if (RedWin%kind == window_counts .or. RedWin%kind == window_arf .or. (RedWin%kind ==window_lensing) ) then ! [CHM] we need this if we compute IA in lensing
        ...
        do i = 1, State%num_redshiftwindows
            associate (RedWin => State%Redshift_W(i))
                ...
                if (RedWin%kind == window_arf) then 
                    window = RedWin%Window%zWindow_f_a(a, winamp) !RedWin%Window%zWindow_f_a(a, winamp)
                endif
                !CHM ____
                !
        ![CHM] The following if statement has been added in order to model IA (eNLA model) in galaxy lensing: ___
        if (RedWin%kind == window_lensing) then 
            RedWin%wing(j) = adot *window
        endif
        ![CHM] ______
		!if ( (RedWin%kind == window_counts) .or. (RedWin%kind == window_arf) ) then ![CHM] counts or arf
        if ( (RedWin%kind == window_counts) .or. (RedWin%kind == window_arf).or. (RedWin%kind == window_vlos) .or. &
               &  ( (RedWin%kind==window_lensing).and.State%CP%SourceTerms%lensing_IA ) ) then ![CHM] counts or arf
                    ...
                    !ARF
                    RedWin%wingz(j) = adot*RedWin%Window%Window_f_a(a, winamp) ! this is the standard dN/da x adot
    ...
    do i = 1, State%num_redshiftwindows
        associate (RedWin => State%Redshift_W(i))
            ...
            if (RedWin%kind == window_arf) then  ! for ARF only
                call spline(TimeSteps%points(jstart),RedWin%Wingz(jstart),ninterp,spl_large,spl_large,RedWin%ddWingz(jstart))
                call spline_deriv(TimeSteps%points(jstart),RedWin%Wingz(jstart),RedWin%ddWingz(jstart), RedWin%dWingz(jstart), ninterp)

                call spline_integrate(TimeSteps%points(jstart),RedWin%Wingz(jstart),RedWin%ddWingz(jstart), RedWin%WinFz(jstart),ninterp)

                RedWin%Fq = RedWin%WinFz(TimeSteps%npoints) 
            ...
            !RedWin%Fq = RedWin%WinF2(TimeSteps%npoints) 
	    !
	        if (RedWin%kind == window_counts) print*,'V4 ADF, window_i  = ',i,State%num_redshiftwindows!,RedWin%z_prom 
            if (RedWin%kind == window_arf) print*,'V4 ARF, window_i  = ',i,State%num_redshiftwindows!,RedWin%z_prom 
            if (RedWin%kind == window_lensing) print*,'V4 lensing, window_i  = ',i,State%num_redshiftwindows!,RedWin%z_prom 
            if (RedWin%kind == window_vlos) print*,'V4 vlos, window_i  = ',i,State%num_redshiftwindows!,RedWin%z_prom 
            !ARF------------------------------------------------------
            ...
            elseif ( ( (RedWin%kind == window_counts) .or. (RedWin%kind == window_arf) ).or. (RedWin%kind == window_lensing) &
                    .or.(RedWin%kind == window_vlos)) then ![CHM] in case of IA within lensing
                if (State%CP%SourceTerms%counts_evolve) then 
        ...
        do RW_i=1, State%num_redshiftwindows
            if (State%Redshift_w(RW_i)%has_lensing_window) then
                associate(W => State%Redshift_W(RW_i), C=> RW(RW_i))
                    ...
                    W%z_prom=C%z_prom
                    W%GFmatter2(j2)=C%GFmatter(i)+d*(C%dGFmatter(i)+d*(3._dl*(C%GFmatter(i+1)-C%GFmatter(i)) &
                        -2._dl*C%dGFmatter(i)-C%dGFmatter(i+1)+d*(C%dGFmatter(i)+C%dGFmatter(i+1) &
                        +2._dl*(C%GFmatter(i)-C%GFmatter(i+1))))) ! matter linear approx. growth factor for IA estimation
                end associate
            end if
        end do
