    ...
    do i = 0, State%num_redshiftwindows
        ...
        if (ThisCT%limber_l_min(s_ix)/=0) then
            ...
            if (i == 0) then
                ...
                if ( (W%kind == window_lensing).and.CP%SourceTerms%lensing_IA) then ! [CHM] We introduce IA here ---
                    s_ix_IA = 3 + W%mag_index + State%num_redshiftwindows
                end if  ! [CHM] -----
            end if
            ...
            do ell = ThisCT%limber_l_min(s_ix), ThisCT%ls%nl
                ...
                do n = n1,n2
                    ...
                    if (k<=qmax) then
                        ...
                        if (s_ix_IA>0) then ! [CHM] We introduce IA contribution --
                            LimbRec%Source(n) = LimbRec%Source(n) + 2 / reall / (reall + 1) * &
                                sqrt(chi * State%TimeSteps%dpoints(n)) * (a0 * ScaledSrc(klo, s_ix_IA, n) + &
                                b0 * ScaledSrc(khi, s_ix_IA, n) + (a03 * ddScaledSrc(klo, s_ix_IA, n) + &
                                b03 * ddScaledSrc(khi, s_ix_IA, n)) * ho2o6)
                        end if  ! [CHM]  --- 
                    else
                        LimbRec%Source(n)=0
                    end if
                        ...
                        do w_ix=1,3 + State%num_redshiftwindows
                            ...
                            if (w_ix>3) then
                                associate (Win => State%Redshift_w(w_ix - 3))
                                    ...
                                    if (Win%kind == window_lensing) then ! [CHM] Introducing IA contribution
                                        Delta1 = Delta1 / 2 * ell * (ell + 1) 
                                        if (CP%SourceTerms%lensing_IA) then
                                            Delta1 = Delta1 + CTrans%Delta_p_l_k(3 + Win%mag_index + &
